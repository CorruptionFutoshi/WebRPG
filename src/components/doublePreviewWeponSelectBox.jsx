...
export const DoublePreviewWeponSelectBox = (props) => {
    ...
    const onClickConfirmButton = () => {
        if (selectedEntity != null && selectedCharacter != null) {
            ...
            const newParty = [];
            selectedCharacter.equip(selectedEntity);
    ...
    return (
        <div>
            ...
            <label isDisabled={targetSelectIsDisabled}>装備させるキャラクターを選択してください</label><br />
            <Select
                isDisabled={targetSelectIsDisabled}
                options={props.party}
                onChange={characterHandleChange}
                styles={customStyles} />
            <label isDisabled={targetSelectIsDisabled}>攻撃力：{selectedCharacter?.attack}＋{selectedEntity?.bonusAttack}　体力：{selectedCharacter?.hp}＋{selectedEntity?.bonusHp}　防御力：{selectedCharacter?.armor}＋{selectedEntity?.bonusArmor}</label><br /><br />
